...
Конфигуриемый http-сканер. Позволяет просканировать список URL'ов на наличие определенных эндпоинтов, отправляя различные запросы (GET, POST и тд). Может обходить проверку от **Cloudflare** (`One moment, please...`), но для этого в системе должна быть установлена **Node.js**.
